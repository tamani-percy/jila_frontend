...
const initializeMap = async () => {
  if (!props.rideRequest || !mapContainer.value) return
  ...
  map.value = L.map(mapContainer.value).setView([pickupLat, pickupLon], 13)
  ...
  L.marker([pickupLat, pickupLon], { icon: mapPinIcon() })
      .addTo(map.value).bindPopup('Your Pickup')

  L.marker([dropoffLat, dropoffLon], { icon: mapPinIcon() })
      .addTo(map.value).bindPopup('Your Drop Off')

  carMarker = L.marker([pickupLat, pickupLon], { icon: carIcon() })
      .addTo(map.value).bindPopup('Your Driver')
  ...
  <div class="flex-1 relative bg-gray-800">
    ...
    <Transition name="fade">
      <div
          ...
          class="absolute top-6 left-1/2 -translate-x-1/2 bg-gray-900/90 backdrop-blur rounded-2xl px-6 py-4 shadow-2xl text-center min-w-[220px]"
      >
        <p class="text-xs text-blue-400 uppercase font-semibold tracking-wide flex items-center justify-center gap-1.5 mb-1">
          <span class="inline-block w-2 h-2 bg-blue-400 rounded-full animate-pulse"/>
          Driver En Route
        </p>
        <p class="text-4xl font-bold text-white tabular-nums transition-all duration-500">
          {{ typeof tripStore.distanceRemaining === 'number'
            ? tripStore.distanceRemaining.toFixed(2)
            : tripStore.distanceRemaining }}
        </p>
        <p class="text-sm text-gray-400 mt-0.5">km away</p>
        <div class="w-full bg-gray-700 rounded-full h-1.5 mt-3">
          <div
              class="bg-blue-500 h-1.5 rounded-full transition-all duration-700 ease-linear"
    ...
    <Transition name="fade">
      ...
      >
        <div class="bg-gray-900 rounded-2xl p-8 text-center shadow-2xl">
          ...
          <p class="text-white text-xl font-bold mt-2">You've arrived!</p>
          <p class="text-gray-400 text-sm mt-1">Thank you for riding with Jila</p>
        </div>
      </div>
    </Transition>
  </div>
